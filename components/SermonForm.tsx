...
import { Youtube, User, Type, Sparkles, Image as ImageIcon, Upload, BookOpen } from 'lucide-react';
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 gap-6">
        ...
        {/* Bible Text Input (NEW) */}
        <div className="space-y-2">
          <label htmlFor="bibleText" className="flex items-center gap-2 text-sm font-medium text-gray-700">
            <BookOpen className="w-4 h-4 text-indigo-500" />
            Texto Bíblico Base (Opcional, mas recomendado)
          </label>
          <input
            type="text"
            id="bibleText"
            ...
            className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all outline-none text-sm shadow-sm"
          />
          <p className="text-xs text-gray-500">Ajuda a IA a encontrar os momentos chaves com precisão teológica.</p>
        </div>
      ...
      <div className="pt-2">
        ...
        <p className="text-center text-xs text-gray-400 mt-3">
          Usando <strong>Gemini 3 Flash</strong> para análise teológica e design visual avançado.
        </p>
      </div>
    </form>
  );
