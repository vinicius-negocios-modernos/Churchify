...
import { Copy, Check, Music, Share2, TrendingUp, Hash, Youtube, BookOpen, Clock, ListChecks, MessageCircle, Download, ImageIcon, HelpCircle, RefreshCw, Loader2, Scissors, Clapperboard, Sparkles } from 'lucide-react';

interface ResultsDisplayProps {
  result: AnalysisResult;
  onRegenerateImage?: (type: 'thumbnail' | 'cover') => Promise<void>;
}

export const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ result, onRegenerateImage }) => {
  ...
  const handleRegenerateClick = async (type: 'thumbnail' | 'cover') => {
    if (!onRegenerateImage) return;
    
    if (type === 'thumbnail') setRegeneratingThumb(true);
    else setRegeneratingCover(true);

    await onRegenerateImage(type);

    if (type === 'thumbnail') setRegeneratingThumb(false);
    else setRegeneratingCover(false);
  };
  ...
  return (
    <div className="space-y-8">
        ...
        <section className="bg-white rounded-xl shadow-sm border border-gray-100">
           <div className="bg-purple-50 px-6 py-4 border-b border-purple-100">
            <h3 className="text-lg font-bold text-purple-800 flex items-center gap-2">
              ...
              Identidade Visual (IA Generativa - Gemini 3 Pro)
            </h3>
          </div>
          ...
          <div className="p-6 grid md:grid-cols-2 gap-8">
             ...
             <div className="space-y-3">
               ...
               {onRegenerateImage && (
                 <button
                   onClick={() => handleRegenerateClick('thumbnail')}
                   disabled={regeneratingThumb}
                   className="w-full py-2 px-4 rounded-lg border border-purple-200 text-purple-700 hover:bg-purple-50 flex items-center justify-center gap-2 text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                 >
                   {regeneratingThumb ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                   {regeneratingThumb ? "Gerando nova versão..." : "Gerar Nova Versão"}
                 </button>
               )}
             </div>
             ...
             <div className="space-y-3">
               ...
               {onRegenerateImage && (
                 <button
                   onClick={() => handleRegenerateClick('cover')}
                   disabled={regeneratingCover}
                   className="w-3/4 md:w-full mx-auto py-2 px-4 rounded-lg border border-purple-200 text-purple-700 hover:bg-purple-50 flex items-center justify-center gap-2 text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                 >
                   {regeneratingCover ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                   {regeneratingCover ? "Gerando nova versão..." : "Gerar Nova Versão"}
                 </button>
               )}
             </div>
          </div>
      ...
      {/* Key Moments Section (Enhanced) */}
      <section className="bg-white rounded-xl shadow-sm border border-gray-100">
         <div className="bg-indigo-50 px-6 py-4 border-b border-indigo-100 flex items-center justify-between">
          <h3 className="text-lg font-bold text-indigo-800 flex items-center gap-2">
            <Scissors className="w-5 h-5" />
            Momentos Chaves (Cortes Virais)
          </h3>
          <span className="text-xs font-bold bg-indigo-100 text-indigo-700 px-2 py-1 rounded flex items-center gap-1 border border-indigo-200">
            <Sparkles className="w-3 h-3" /> IA: Gemini 3 Flash
          </span>
        </div>
        ...
        <div className="p-6 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {result.keyMoments.map((moment, idx) => (
            <div key={idx} className="bg-white rounded-lg border border-gray-200 p-5 hover:shadow-md transition-shadow flex flex-col h-full relative overflow-hidden">
              {/* Viral Score Badge */}
              <div className={`absolute top-0 left-0 ${getScoreColor(moment.viralScore)} text-xs font-bold px-3 py-1 rounded-br-lg flex items-center gap-1 z-10`}>
                 <TrendingUp className="w-3 h-3" />
                 Viral Score: {moment.viralScore}
              </div>

              <div className="absolute top-0 right-0 bg-gray-100 text-gray-600 text-xs font-bold px-3 py-1 rounded-bl-lg flex items-center gap-1">
              ...
              <div className="space-y-4 flex-grow">
                 <div>
                    <p className="text-xs font-semibold text-gray-500 uppercase flex items-center gap-1">
                      <Clapperboard className="w-3 h-3" /> Direção Visual
                    </p>
                    <p className="text-xs text-blue-600 mt-1 bg-blue-50 p-2 rounded border border-blue-100">
                      {moment.visualStyle}
                    </p>
                 </div>
                 ...
                 <div className="pt-3 mt-2 border-t border-gray-100">
                    <p className="text-xs font-semibold text-gray-500 uppercase">Por que viraliza?</p>
