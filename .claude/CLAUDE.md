...
|-----------|-------------|
| `5-core-rules.md` | 5 regras inegociaveis (delegar, evidencia, root cause, limites, memory) |
| `context-management.md` | Gestao de contexto, subagentes, model selection, counter protocol |
| `delegation-enforcement.md` | Decision matrix, prompt templates, anti-patterns, batching |
...
> Regras validadas em ~360 sessoes reais (Central MKT). Guia completo: `docs/guides/aiox-context-optimization-guide.md`

### 5 Core Rules (INEGOCIAVEIS)
...
3. **Root Cause Antes de Fix** — 1 commit resolve tudo, zero fix incremental
4. **Subagentes com Limite** — Context counter: `[context_rounds = N/40 · subagents = M/30 · GREEN/YELLOW/RED]`
5. **MEMORY = Estado Real** — Secoes BLOCKING + Estado Atual obrigatorias

### Delegation Rule
- **2+ tool calls = delegar a subagente** (via Task tool com AIOX agent identity)
- Orion so executa direto: 1 Read, 1 git status/log, 1 Grep/Glob
- **Read-to-Edit Trap:** Se vai editar, delegar a task INTEIRA ao agente responsavel

### Subagent Guidelines
- Model: `opus` (julgamento) ou `haiku` (mecanico). NUNCA `sonnet`
- 1 subagente = 1 arquivo ou 1 concern. Max 5 por batch. Max 40 linhas output
...
### Behavioral Rules
**NEVER:**
- Edit app code directly as Orion
- Run git commit/push as Orion
- Launch parallel npm test/lint/typecheck
- Build tables from 4+ subagent results in main context
- Save MEMORY.md before RED threshold

**ALWAYS:**
- Show context counter before each batch
- Use Task tool for 2+ tool call operations
- Include full AIOX agent identity in subagent prompts
- Serialize QA gates and heavy Node.js operations
