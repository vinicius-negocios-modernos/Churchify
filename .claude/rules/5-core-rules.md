# 5 Core Rules — AIOX Context Optimization

> Estas 5 regras sao o MINIMO INEGOCIAVEL em TODA sessao. Origem: ~360 sessoes, Central MKT.

## 1. Delegar, Nunca Executar

Orion coordena. Code vai para @dev, git/deploy para @devops, schema para @data-engineer, QA para @qa. Sem excecoes.

**Verificacao:** Nenhum `Edit`, `Write` em codigo de aplicacao, nem `git commit/push` executado diretamente por Orion.

**Violacao comum:** Orion le um arquivo "para investigar" e acaba editando diretamente.

## 2. Evidencia, Nao Intencao

Nada esta "feito" sem output verificavel. CI PASS != prod OK. Deploy triggered != deploy funcionando.
...
Diagnosticar o problema COMPLETO antes do primeiro commit. Zero fix incremental sem entender todas as causas.
...
Todo trabalho via Task tool. Contador explicito declarado antes de cada batch.
...
MEMORY.md registra apenas fatos verificados com evidencia. "Pendente" exige descricao do que falta. "Completo" exige prova.
